...
public final class SignSaverFileUnsafe implements SignSaver {
    ...
    public SignSaverFileUnsafe(final String targetFileName) {
    ...
    public void saveSign(final SingleSign sign, final byte[] dataToSave) throws IOException {
        if (!DISABLED) {
            LOGGER.info("Comienzo de guardado de la firma '" + sign.getId() + "' en: " + this.filename); //$NON-NLS-1$ //$NON-NLS-2$
            try {
